...
export default function LoginPage() {
  ...
  return (
    <div className="mx-auto max-w-md">
      ...
      <p className="mt-2 text-sm text-slate-400">Access your Channel Connect account.</p>

      <form onSubmit={handleSubmit} className="mt-8 space-y-4">
        {error && <p className="rounded-md bg-red-950/60 p-3 text-sm text-red-300">{error}</p>}
        {notice && <p className="rounded-md bg-brand-surface p-3 text-sm text-brand-light">{notice}</p>}
        <div>
      ...
      <p className="mt-6 text-center text-sm text-slate-400">
        No account? <Link href="/register" className="text-brand-light hover:underline">Sign up</Link>
      </p>
    </div>
  );
}
