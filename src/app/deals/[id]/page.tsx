  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-bold text-slate-100">{deal.title}</h1>
        <p className="mt-2 text-slate-400">{deal.description}</p>
      </div>
          ...
          <ul className="mb-4 space-y-3">
            {deal.messages.map((m) => (
              <li key={m.id} className={`rounded-lg p-3 text-sm ${m.senderId === user.id ? "bg-brand-surface" : "bg-navy-elevated"}`}>
                <p className="font-medium text-slate-100">{m.sender.name}</p>
                <p className="mt-1 text-slate-300">{m.body}</p>
